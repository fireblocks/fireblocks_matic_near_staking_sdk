...
import { Staker } from "../dist/Staker";
...
class CustomStaker extends Staker{
     ...
     */
    constructor(fbks: FireblocksSDK, vaultAccountId: number, web3?: Web3/*, parameter4: type4, parameter5: type5, ... */){
    ...
    - It is recommended that you use:
      * typecheck(value: any, type: string)     - to validate types.
      * await runSetupIfNeeded()                - to make sure all class-scoped values are defined and there is no issue with user input.
      * this.standardTx(dest: DestinationTransferPeerPath, extraParameters: object, note?: string, amount?: string, operation? : TransactionOperation = TransactionOperation.CONTRACT_CALL) - to generate the basic transaction payload.
      * await this.issueTx(txPayload: TransactionArguments) - to send a transaction to the fireblocks infrastructure. The returns value is the transaction id.
    ...
    getBalances(wei: boolean): Promise<number[]> {
        /**
